 ...
 * with Hedera Fungible Tokens, like creating and deleting Token. An implementation of this
 * interface is using an internal account to interact with a Hiero network. That account is the
 * account that is used to pay for the transactions that are sent to the Hedera network and called
 * 'operator account'.
 */
public interface FungibleTokenClient {

  /**
   * Create a new token. The operator account is used as suppler account and as treasury account for
   * the token.
   *
